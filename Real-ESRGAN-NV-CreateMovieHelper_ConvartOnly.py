'''
    ...
    # This file is Convart Only !!
    # もし、なにかしらの不具合で動画連結できなかった場合に使用してください。
    # 例:h264のコーデックで5K以上の動画を連結しようとした場合、エラーになる等。
    #    H265のコーデック等にしたうえで、こちらのプログラムを使用。
...
def Combining_Picture():
    ...
    cmd_text = 'ffmpeg -framerate ' + fps + ' -i .\output_frames\%08d.png -i "' + basename + '" -map 0:v:0 -map 1:a:0 -strict -2 -vcodec ' + Codec + ' -acodec copy -b:v ' + Bitrate + ' -x264opts colorprim=bt709:transfer=bt709:colormatrix=smpte170m -r ' + fps + ' "'+ basename_without_ext +'_enhanced.mp4"'
    subprocess.call(cmd_text, shell=True)
